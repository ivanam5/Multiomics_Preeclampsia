...
foldidAll<- read.csv(file="/Users/IvanafoldidAll.csv")
...
##############################################
numbercovm <- c(5,8,10,20,25,30,40,56) # chosen number of covariates 
rocmetaburine_all <- numeric(length(numbercovm))

for (k in 1:length(numbercovm)){
    print(k)   
set.seed(300)
# Initialize 
...
rocmetaburine_all[k]<- as.numeric(roc10metaburine$auc)
print(rocmetaburine_all)
} #for
...
############################################
numbercov <- c(8,10,20,25,30,36) # chosen number of covariates
rocproteome_all <- numeric(length(numbercov))

for (k in 1:length(numbercov)){
print(k)
set.seed(300)
# Initialize 
S               <- proteom_data[,c("SampleId", "individual", "PE")]
...
t_modelpe <- t_modelpe[-1,] #remove intercept
 ...
 t_modelpe$absvalue <- abs(t_modelpe$value)
 t_modelpe <- t_modelpe[order(t_modelpe$absvalue, decreasing=TRUE),]
 t_modelpe10 <- t_modelpe[c(1:numbercov[k]),]
 t_modelpe10 <-t_modelpe10[!is.na(t_modelpe10$column),]
 print(nrow(t_modelpe10 ))
...
rocproteome_all[k] <- as.numeric(roc10proteome$auc)
print(rocproteome_all)
} #for

plot(numbercovm, rocmetaburine_all,   type="b", col="#3F51B5", xlab="Number of covariates", ylab="AUC",  lwd=3, cex.lab=1.7, xlim=c(0,60), ylim=c(0.6, 1))
lines(numbercov, rocproteome_all, type="b",col="#F57C00", lwd=2)
legend("bottomright", legend=c("Urine metabolome", "Proteome"), col = c("#3F51B5", "#F57C00"), lty = 1.5, cex = 1.5)
